...
doctorRoute.post("/register", async (req, res) => {
    let { name, email, password, role, specialization, location, image, videoCall } = req.body
    ...
    if (doctorFound) {
        ...
        try {
            ...
            bcrypt.hash(password, 5, async function (err, hash) {
                const data = new DoctorModel({ name, email, password: hash, image, registeredDate: dateFormat, role, specialization, location, videoCall })
                await data.save()
...
doctorRoute.get("/all", async (req, res) => {
    try {
        ...
        if (specialty && specialty.toUpperCase() !== "ALL") {
            query.specialization = specialty.toUpperCase();
        }

        if (location && location.toUpperCase() !== "ALL") {
            query.location = location.toUpperCase();
        }

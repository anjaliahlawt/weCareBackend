...
patientRoute.post("/register", async (req, res) => {
    const { name, email, password, role, image, location } = req.body
    ...
    if (patientFound) {
        ...
        try {
            ...
            bcrypt.hash(password, 5, async function (err, hash) {
                const data = new PatientModel({ name, email, password: hash, image, registeredDate: dateFormat, role, location: upperCaseLocation })
                await data.save()
